...
/**
 * BearMap3D
 * - Satellite basemap (Esri World Imagery, no key)
...
export default function BearMap3D({
  showBears = true,
  showHabitat = true,
  showStateWires = false, // expects /public/data/us_states.geojson if enabled
}) {
  ...
  useEffect(() => {
    ...
    map.on("load", async () => {
      ...
      // Helpful: auto-fit to data once (so you see clusters right away)
      try {
  ...
  return (
    <div ref={containerRef} className="stage" style={{ width: "100%" }} />
  );
}
